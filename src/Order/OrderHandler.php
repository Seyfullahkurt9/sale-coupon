...
class OrderHandler {
	...
	public function process_order_coupons( $order_id ) {
		try {
			...
			// Log error so that the site doesn't crash during payment callback/webhook
			error_log( sprintf( '[Sale Coupon Error] process_order_coupons failed for Order #%d: %s in %s on line %d', $order_id, $e->getMessage(), $e->getFile(), $e->getLine() ) );
			
			if ( function_exists( 'wc_get_logger' ) ) {
				wc_get_logger()->error( 
					sprintf( 'Order #%1$d coupon generation failed: %2$s', $order_id, $e->getMessage() ), 
					[ 'source' => 'sale-coupon' ] 
				);
			}
			...
			if ( ! empty( $order ) ) {
				$order->add_order_note( 
					sprintf( 
						__( 'Kupon üretimi sırasında hata oluştu: %s. Detaylar sistem loglarındadır.', 'sale-coupon' ), 
						$e->getMessage() 
					) 
				);
			}
